...
class Shop {
  ...
  final String RegisterId; // Added RegisterID
  ...
  Shop({
    ...
    required this.RegisterId, // Initialize RegisterID
  });
}
      ...
      setState(() {
        _shops = snapshot.docs.map((doc) {
          ...
          return Shop(
            ...
            process: data['ShopProcess'] ?? '', // Assuming 'ShopProcess' is the correct field name for the process
            prizes: data['Prizes'] ?? '', // Adjust the field name as per your Firebase structure
            shopAddress: data['ShopAddress'] ?? '',
            status: data['ShopStatus'] ?? '', // Fetch shop status from Firestore
            RegisterId: data['RegisterId'] ?? '', // Fetch RegisterID from Firestore
          );
        }).toList();

        if (_isSearching && _searchController.text.isNotEmpty) {
          _shops.sort((a, b) => a.name.startsWith(_searchController.text) ? -1 : 1);
        }
      });
    } catch (e) {
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              Text('RegisterID: ${shop.RegisterId}'), // Display RegisterID
            ],
          ),
          actions: [
            TextButton(
                      ...
                      Positioned(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color: _shops[index].status == 'available' ? Colors.green : Color.fromARGB(255, 55, 0, 255),
